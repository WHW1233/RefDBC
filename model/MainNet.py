...
class MergeTail(nn.Module):
    ...
    def forward(self, x2, x3):
        ...
        x = F.relu(self.conv_merge(torch.cat((x3, x23), dim=1)))
        x = self.conv_tail1(x)
        x = self.conv_tail2(x)
        x = torch.clamp(x, -1, 1)
...
class MainNet(nn.Module):
    def __init__(self, num_res_blocks, n_feats, res_scale):
        ...
        ### stage11
        self.conv11_head = conv3x3(128+n_feats, n_feats)
    ...
    def forward(self, x, S=None, T_lv2=None, T_lv1=None):
        ...
        ### soft-attention
        x11_res = x11
        x11_res = torch.cat((x11_res, T_lv2), dim=1)
        ...
        ### soft-attention
        x22_res = x22
        x22_res = torch.cat((x22_res, T_lv1), dim=1)
        ...
        # x32 = x32 + x32_res
        # x33 = x33 + x33_res
        # print('X31, x32, x33 shape', x31.shape, x32.shape, x33.shape)

        x = self.merge_tail(x32, x33)
