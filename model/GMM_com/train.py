...
import json
import time
from model.GMM_com.datasets import Datasets, TestKodakDataset
from tensorboardX import SummaryWriter
from model.GMM_com.Meter import AverageMeter
import pdb
...
def testKodak(step):
    with torch.no_grad():
        ...
        sumMsssimDB = 0
        cnt = 0
        for batch_idx, input_list in enumerate(test_loader):
            input = input_list[0]
            name = input_list[1]
            print(name)
            clipped_recon_image, mse_loss, bpp_feature, bpp_z, bpp = net(input)
            ...
            im.save('./data/compressed/{}'.format(name[0]))
