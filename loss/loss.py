...
class TPerceptualLoss(nn.Module):
    ...
    def forward(self, map_lv2, map_lv1, S, T_lv2, T_lv1):
        ### S.size(): [N, 1, h, w]
        if (self.use_S):
            S_lv2 = torch.sigmoid(S)
            S_lv1 = torch.sigmoid(F.interpolate(S, size=(S.size(-2)*2, S.size(-1)*2), mode='bicubic'))
...
class Hconsistency(nn.Module):
    def __init__(self, H=128,W=128, type='l1'):
        super(Hconsistency, self).__init__()
        self.H = H
        self.W = W
        self.type = type
    ...
    def forward(self, H):
        ### H.size(): [N, 1, h*w]
        N = H.shape[0]
        X = [i for i in range(self.H*self.W)]
        L = torch.tensor([X])
        L = L.repeat(N, 1, 1)
        print(L.size)

        if (self.type == 'l1'):
            cons_loss = F.l1_loss(H, L)
        elif (self.type == 'l2'):
            cons_loss = F.mse_loss(H, L)
...
def get_loss_dict(args, logger):
    ...
    if (abs(args.cons_w - 0) > 1e-8):
        loss['cons_loss'] = Hconsistency(H=128,W=128, type='l1')
    return loss