...
class TrainSet(Dataset):
    def __init__(self, args, transform=transforms.Compose([RandomFlip(), RandomRotate(), ToTensor()]) ):
        self.input_list = sorted([os.path.join(args.dataset_dir, 'train/input', name) for name in 
            os.listdir( os.path.join(args.dataset_dir, 'train/input') )])       #排序的
    ...
    def __getitem__(self, idx):
        ...
        LR = np.array(Image.fromarray(hr_img).resize((w//DOWN_SCALE, h//DOWN_SCALE), Image.BICUBIC))      #4倍降采样
        LR_sr = np.array(Image.fromarray(LR).resize((w, h), Image.BICUBIC))         #4倍上采用
...
class TestSet(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ### LR and LR_sr
        LR = np.array(Image.fromarray(HR).resize((w//DOWN_SCALE, h//DOWN_SCALE), Image.BICUBIC))
